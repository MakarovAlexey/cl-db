...
** Query building
   ...
   слотов).
*** Структура запросов
    Проверка структуры запросов - от сложного к простому
**** query-with-3 - запрос с тремя CTE (похоже максимальное количество)
     - есть обе рекурсивные части (join и fetch)
     - есть рекурсивная часть join, fetch и одно из условий выборки,
       наличие агрегатной функции, where, having , limit или offset.
**** query-with-2 - зарпос с двумя CTE
     - нет recursive join, но есть limit или offset и recursive fetch
**** query-with-1 - запрос с одним CTE
     - есть recursive join и recursive fetch, но в select-list нет
       агрегатной функции, а также нет where и having отсуттвуют условия 
     - 
**** simple query - простой запрос без WITH
     - отсутствие рекурсивных частей
*** Классы
   |                   | superclass-nodes | fetched-references | subclass-nodes | foreign-key | join-references | properties |
   |-------------------+------------------+--------------------+----------------+-------------+-----------------+------------|
   | root-node         | x                | x                  | x              |             | x               | x          |
   | joined-reference  | x                | x                  | x              | x           | x               | x          |
   | subclass-node     | x                | x                  | x              | x           |                 |            |
   | fetched-reference | x                | x                  | x              | x           |                 |            |
   | superclass-node   | x                | x                  |                | x           | x               |            |
...
** Рекурсиваня выборка
   ...
   |           | limit | offset | join | recursive | roots |
   |-----------+-------+--------+------+-----------+-------|
   | fetch     | x     | x      | -    | x         | -     |
   | r. fetch  | x     | x      | x    | x         | x     |
   | recursive | -     | -      | -    | x         | -     |
...
** selection class
   содержит два слота: спсиок выражений списка выбора (select-list) и
   from-clause
